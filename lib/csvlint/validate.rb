module Csvlint
  ...
  class Validator
    ...
    ERROR_MATCHERS = {
        ...
        "Any value after quoted field isn't allowed" => :unquoted_character
    }
    ...
    def locate_schema
      ...
      case @source
        when StringIO
          return
        when File
          @source_url = "file:#{Addressable::URI.encode(File.expand_path(@source))}"
        else
          @source_url = @source
      end
      unless @schema.nil?
      ...
      if @source_url =~ /^http(s)?/
        begin
          ...
          paths = Net::HTTP.get(well_known_uri).split("\n")
